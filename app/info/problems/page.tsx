...
// =======================================================
// [CONFIG] Cấu hình API và Model MỚI: Problem
// =======================================================
...
interface Problem {
  id: string | number;
  code: string; // Tương ứng với problem_code (FE: Code/Mã)
  name: string; // Tương ứng với problem_name (FE: Name/Tên)
}

// Khai báo kiểu dữ liệu mà API Django trả về/mong đợi (ApiProblem)
interface ApiProblem {
    id: string | number;
    problem_code: string; // Tên trường từ Django Model
    problem_name: string; // Tên trường từ Django Model
}
...
const mapApiToFrontend = (apiItem: ApiProblem): Problem => {
    return {
        id: apiItem.id,
        code: apiItem.problem_code, // Map problem_code -> code
        name: apiItem.problem_name, // Map problem_name -> name
    };
};

// Hàm chuyển đổi từ Frontend sang API (CREATE/UPDATE)
const mapFrontendToApi = (feData: Omit<Problem, 'id'>): Omit<ApiProblem, 'id'> => ({
    problem_code: feData.code, // Map code -> problem_code
    problem_name: feData.name, // Map name -> problem_name
});
...
// =======================================================
// [MAIN COMPONENT] ProblemManager
// =======================================================

export default function ProblemManager() {
  // State chứa danh sách Problem
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const frontendData: Problem[] = apiData.map(mapApiToFrontend);
  ...
  const validateForm = () => {
    ...
    // Kiểm tra trùng lặp Code (giả định code là unique)
    const isCodeDuplicate = items.some(
      item => item.code.trim() === formData.code.trim() && item.id !== editingItem?.id
    );
    if (isCodeDuplicate) {
        setError(`Mã Lỗi "${formData.code.trim()}" đã tồn tại cục bộ. Vui lòng kiểm tra lại.`);
        return false;
    }
    return true;
  };
  ...
  const handleServerError = async (response: Response) => {
      ...
      // KIỂM TRA LỖI THEO TÊN TRƯỜNG API MỚI: problem_code và problem_name
      if (errorData.problem_code && errorData.problem_code[0]) {
          message = `Lỗi Validation (Mã Lỗi): ${errorData.problem_code[0]} (Mã có thể bị trùng hoặc không hợp lệ).`;
      } else if (errorData.problem_name && errorData.problem_name[0]) {
          message = `Lỗi Validation (Tên Lỗi): ${errorData.problem_name[0]}.`;
  ...
  // ------------------------------------
  // [CRUD - CREATE] Thêm mới Problem (POST)
  // ------------------------------------
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const newItemApi: ApiProblem = await response.json();
        const newItemFrontend: Problem = mapApiToFrontend(newItemApi);
  ...
  // ------------------------------------
  // [CRUD - UPDATE] Cập nhật Problem (PUT)
  // ------------------------------------
  const handleUpdateItem = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const updatedItemApi: ApiProblem = await response.json();
        const updatedItemFrontend: Problem = mapApiToFrontend(updatedItemApi);
  ...
  const handleStartEdit = (item: Problem) => {
    setEditingItem(item);
    setFormData({
      code: item.code, // Đã đổi từ 'name'
      name: item.name, // Đã đổi từ 'description'
    });
    setError('');
  ...
  // ------------------------------------
  // [CRUD - DELETE] Xóa Problem (DELETE)
  // ------------------------------------

  // Chuẩn bị xóa (Mở modal)
  const handlePrepareDelete = (item: Problem) => {
  ...
  const confirmDelete = async () => {
      ...
      try {
        ...
        setError("Lỗi hệ thống: Không thể xóa Problem.");
      } finally {
  ...
  const FormComponent = (
    <div className="bg-white shadow-xl rounded-xl p-6 mb-8 border border-blue-100">
      <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center">
        {editingItem ? (
            <>
                <EditIcon /> Chỉnh sửa Lỗi: <span className="text-blue-600 ml-2">{editingItem.code}</span>
            </>
        ) : (
            <>
                <PlusIcon /> Thêm Lỗi (Problem) Mới
            </>
        )}
      </h2>
      ...
      <form onSubmit={editingItem ? handleUpdateItem : handleAddItem} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          {/* Mã Lỗi (problem_code) */}
          <div>
            <label htmlFor="code" className="block text-sm font-medium text-gray-700 mb-1">Mã Lỗi (Code)</label>
            <input
              id="code"
              name="code"
              type="text"
              ...
              placeholder="Ví dụ: P001-HUNGMAY"
          ...
          {/* Tên Lỗi (problem_name) */}
          <div className="md:col-span-2">
            <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-1">Tên Lỗi/Vấn Đề</label>
            <input
              id="name"
              name="name"
              type="text"
              value={formData.name}
              onChange={handleChange}
              placeholder="Mô tả ngắn gọn về lỗi (Ví dụ: Hư hỏng cơ khí)"
              className="w-full p-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition duration-150"
              required
              disabled={isLoading}
            />
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      ...
      <div className="bg-white shadow-xl rounded-xl overflow-hidden border border-gray-200">
        <div className="p-4 flex justify-between items-center bg-blue-50 border-b border-blue-100">
            <h2 className="text-xl font-bold text-gray-800">Danh Sách Lỗi ({items.length} mục)</h2>
             <button
                onClick={fetchItems}
                disabled={isLoading}
        ...
        {isLoading && items.length === 0 ? (
            ...
            <div className="overflow-x-auto">
                ...
                <thead className="bg-gray-100">
                    <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Mã Lỗi (Code)</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tên Lỗi/Vấn Đề</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        {error ? 'Không thể tải dữ liệu.' : 'Chưa có mã lỗi nào được thêm.'}
                        </td>
                    </tr>
                    ) : (
                    items.map((item) => (
