  ...
  return (
    <>
      <tr className="bg-slate-100 border-b border-slate-300">
        <td colSpan={days + 2} className="px-2 py-1.5 font-black text-[10px] text-blue-900 sticky left-0 z-20 bg-slate-100 uppercase border-r border-slate-300">
          {title}
        </td>
      </tr>
        ...
        <>
          {keys.map((key) => {
            ...
            return (
              <tr key={key} className="hover:bg-blue-50/50 transition-colors border-b border-slate-200 group">
                ...
                <td className="p-2 sticky right-0 bg-blue-50 group-hover:bg-blue-100 z-10 border-l border-slate-300 font-black text-right text-blue-800 text-[10px] min-w-[70px] w-[70px]">
                  {finalTotal !== null ? (typeof finalTotal === 'number' ? finalTotal.toLocaleString() : finalTotal) : ""}
                  {key.toLowerCase().includes('percent') && Number(finalTotal) > 0 ? '%' : ''}
                </td>
              </tr>
            );
          })}
          ...
          {type === 'number' && showTotal && (
            <tr className="bg-blue-50/50 border-b-2 border-slate-400 font-black italic">
              <td className="p-2 sticky left-0 bg-blue-100/90 z-10 border-r border-slate-300 text-[10px] text-blue-900 min-w-[200px]">
              ...
              <td className="p-2 sticky right-0 bg-blue-600 z-10 border-l border-blue-700 text-right text-white text-[10px] min-w-[70px] w-[70px]">
