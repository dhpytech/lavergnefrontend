...
// Tông màu Indigo & Slate cho Metal Dashboard
const COLORS = ['#6366f1', '#475569', '#f59e0b', '#ef4444', '#8b5cf6'];
...
export default function MetalDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Đã cập nhật endpoint sang /metal/
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/dashboard/metal/?${query}`);
      const json = await res.json();
      setData(json);
    } catch (error) {
      console.error("Lỗi fetch Metal Dashboard:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 bg-[#f8fafc] min-h-screen space-y-6">
      ...
      {/* --- HEADER TIÊU ĐỀ METAL --- */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="w-2 h-2 rounded-full bg-indigo-600 animate-pulse"></span>
            <span className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em]">Metal Line Monitoring</span>
          </div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight uppercase">
            METAL <span className="text-indigo-600">DASHBOARD</span>
          </h1>
          <p className="text-xs text-slate-400 font-medium italic">Advanced metallurgy production metrics & lot analysis</p>
        </div>

        <div className="flex items-center gap-3">
          <div className="text-right hidden md:block">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-1">Data Source</p>
            <p className="text-xs font-black text-slate-600">Metal Production API</p>
          </div>
          ...
          <div className="bg-slate-50 border border-slate-200 px-4 py-2 rounded-xl text-center min-w-[120px]">
             <span className="text-[10px] font-black text-slate-400 uppercase block leading-none mb-1">System Status</span>
             <span className="text-xs font-black text-indigo-600 uppercase">Operational</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-200 flex flex-wrap gap-4 items-center">
        <div className="flex items-center gap-2">
          ...
          <select
            className="border rounded-md px-2 py-1 text-xs font-bold outline-none bg-slate-50 focus:ring-2 focus:ring-indigo-500"
            value={filters.shift}
        ...
        <button
          ...
          disabled={loading}
          className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-1.5 rounded-md font-black text-[10px] uppercase tracking-widest transition-all disabled:opacity-50 flex items-center gap-2 ml-auto shadow-sm"
        >
          {loading ? (
            <>
        ...
        <>
          ...
          <div className="grid grid-cols-12 gap-6">
            <div className="col-span-12 lg:col-span-5 bg-white p-6 rounded-2xl border border-slate-100 h-[350px] shadow-sm">
              <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-4">Metal Shipping Mix</h3>
            ...
            <div className="col-span-12 lg:col-span-7 bg-white p-6 rounded-2xl border border-slate-100 h-[350px] shadow-sm">
              <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-4">Monthly Production Trend</h3>
              <ResponsiveContainer width="100%" height="85%">
                <BarChart data={data.charts.bar_chart}>
                  ...
                  <Bar dataKey="volume" fill="#4f46e5" radius={[4, 4, 0, 0]} barSize={30} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm h-[400px]">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Metal Lot Analysis - {lotView.toUpperCase()}</h3>
                ...
                {(['shipping', 'input', 'reject'] as const).map(mode => (
                  <button
                    ...
                    className={`px-4 py-1 text-[10px] font-black rounded-md transition-all ${
                      lotView === mode ? 'bg-slate-700 text-white shadow-md' : 'text-slate-500 hover:bg-slate-200'
                    }`}
                  >
                    {mode.toUpperCase()}
                  </button>
            ...
            <ResponsiveContainer width="100%" height="85%">
              <BarChart data={data?.charts?.lot_chart}>
                ...
                <Bar
                  dataKey={lotView}
                  fill={lotView === 'reject' ? '#dc2626' : lotView === 'input' ? '#4338ca' : '#059669'}
                  radius={[4, 4, 0, 0]}
                  barSize={60}
                />
              </BarChart>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="p-4 border-b bg-slate-50 flex justify-between items-center">
               <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Metal Production Detail by Lot</span>
            </div>
            <div className="overflow-x-auto max-h-[400px]">
              <table className="w-full text-left">
                <thead className="sticky top-0 bg-white border-b text-[10px] font-black text-slate-400 uppercase">
                  <tr>
                    <th className="p-4 text-indigo-600">Lot Number</th>
                    <th className="p-4 text-right">In (Unit)</th>
                    <th className="p-4 text-right">Out (Unit)</th>
